...
export function corridorBeats() {
  ...
  /* An inset has no roads between the boundary and its markers, so the
     offices land as the boundary settles rather than waiting out a draw
     that never happens. */
  const offices = outline + MAP.outlineDuration - DUR.lift;

  return { outline, context, corridor, nodes, offices };
}
